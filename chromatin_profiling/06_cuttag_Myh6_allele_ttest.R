...
if (bin_size %in% c(50, 100, 500)){
  bg_file <- list.files(data.path, pattern = paste0(".bin", bin_size, ".seqdepth_scaled.bedgraph"), full.names = F)
} else{
  stop(paste0("bin_size should be 50, 100 or 500"))
}


bg_file <- list.files(data.path, paste0(".bin", bin_size, ".seqdepth_scaled.bedgraph"), full.names = F)
...
for (sample in samples) {
  for (group in c("total", "c57", "dba")) {
    bedgraphs <- list.files(data.path, pattern = paste0(sample, ".*", group, ".bin", bin_size, ".seqdepth_scaled.bedgraph"), 
                            full.names = TRUE)
    matrix_list <- list()
    for (bedgraph in bedgraphs) {
      ...
      samplename <- sub(paste0("(.*).", group, ".bin", bin_size, ".seqdepth_scaled.bedgraph"), "\\1", basename(bedgraph))
