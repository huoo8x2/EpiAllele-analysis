...
track_plot = function(summary_list = NULL,
  ...
  condition = summary_list$colData[,get(condition)]
  summary_list = summary_list$data
  ...
  if(groupScaleByCondition){
    ...
    plot_height = data.table::data.table(plot_height, plot_height_min, condition, names(summary_list))
    plot_height$og_ord = 1:nrow(plot_height)
    plot_height = plot_height[order(condition)]
    plot_height_max = plot_height[,.(.N, max(plot_height)), .(condition)]
    plot_height_min = plot_height[,.(.N, max(plot_height_min)), .(condition)]
