...
#
# Description:
# This script performs the downstream analysis for bulk RNA-seq data, including:
#   - Mapping quality assessment
#   - Expression matrix generation and TPM normalization
#   - PCA analysis
#   - Differential expression analysis 
#   - GO and KEGG enrichment analysis 
#   - Sample expression correlation analysis
#   - Heatmap generation for overall expression and differentially expressed genes
#
# Required Arguments:
#   --result_path / -r   Path to the result directory containing both input and output files
#   --species     / -s   Species name: "human" or "mouse"
#
# Example:
#   Rscript 03_downstream_analysis.R --result_path results/bulk_rnaseq --species human
#
# Output:
#   The script generates normalized expression matrix, PCA plot, expression heatmap, 
#   DEG tables and enrichment results.
#
# =============================================================================
...
spec <- matrix(c(
  ...
  'result_path', 'r', 1, "character", 'Path to the results directory (for input and output)', 
  'species', 's', 1, "character", 'Species name ("human" or "mouse")'
), byrow=TRUE, ncol=5)
...
show_custom_help <- function(spec) {
  cat("\n=============================================================================\n\n")
  cat("Description:\n")
  cat("  This script performs the downstream analysis for bulk RNA-seq data, including:\n")
  cat("    - Mapping quality assessment\n")
  cat("    - Expression matrix generation and TPM normalization\n")
  cat("    - PCA analysis\n")
  cat("    - Differential expression analysis\n")
  cat("    - GO and KEGG enrichment analysis\n")
  cat("    - Sample expression correlation analysis\n")
  cat("    - Heatmap generation for overall expression and differentially expressed genes\n")
  cat("\n-----------------------------------------------------------------------------\n")
  
  # Call getopt to print the standard usage and argument list.
  # This ensures the argument list always stays in sync with the spec matrix.
  cat(getopt(spec, usage = TRUE))
  
  cat("-----------------------------------------------------------------------------\n\n")
  cat("Output:\n")
  cat("  The script generates a normalized expression matrix, PCA plot, expression heatmap,\n")
  cat("  DEG tables, and enrichment analysis results.\n\n")
  
  cat("Example:\n")
  cat("  Rscript 03_downstream_analysis.R --result_path results/bulk_rnaseq --species human\n")
  cat("\n=============================================================================\n\n")
}
...
# library packages ----
for (p in c("tidyverse", "stringr", "ggrepel", "EnhancedVolcano", "DESeq2", "ggsci", 
...
# create results.path and load colorSet ----
dir.create(results.path, showWarnings = F)
featureCount.path <- paste0(results.path, "/featureCount")
...
##gtf:
#mouse:https://ftp.ensembl.org/pub/release-113/gtf/mus_musculus/Mus_musculus.GRCm39.113.chr.gtf.gz
#human:https://ftp.ensembl.org/pub/release-113/gtf/homo_sapiens/Homo_sapiens.GRCh38.113.chr.gtf.gz

if (species == "human") {
  gtf <- rtracklayer::import("ref/human/GRCh38/Homo_sapiens.GRCh38.113.chr.gtf") %>% 
    as.data.frame()
} else if (species == "mouse") {
  gtf <- rtracklayer::import("ref/mouse/GRCm39/Mus_musculus.GRCm39.113.chr.gtf") %>% 
    as.data.frame()
} else {
...
## replicates mean expression correaltion ----
...
source("/data02/hukaijie/EpiAllele/final_script/bulk_RNAseq/03_DE_function.R")
...
source("/data02/hukaijie/EpiAllele/final_script/bulk_RNAseq/03_enrichment_function.R")
