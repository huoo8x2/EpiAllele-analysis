...
## remove C/T mutation SNPs: 164/465 SNPs
def is_ct_conflict(dba, c57):
    dba_set = set(dba.split("/"))
    c57_set = set(c57.split("/"))
    return ('T' in dba_set and 'C' in c57_set) or ('C' in dba_set and 'T' in c57_set)

snp_filter = snp_mp[~snp_mp.apply(lambda row: is_ct_conflict(row['DBA/2J'], row['C57BL/6J']), axis=1)].copy()
...
for read in samfile:
    ...
    for query_idx, ref_pos in read.get_aligned_pairs():
        ...
        if ref_pos in snp_positions:  # Check if the reference position is a SNP
            ...
            allele = snp_filter.loc[snp_filter['position_0_base'] == ref_pos, 'Observed'].values[0]
            dba_base = snp_filter.loc[snp_filter['position_0_base'] == ref_pos, 'DBA/2J'].values[0].split("/")
            c57_base = snp_filter.loc[snp_filter['position_0_base'] == ref_pos, 'C57BL/6J'].values[0].split("/")
            if "C" in dba_base:
                 dba_base = dba_base + ["T"]
            if "C" in c57_base:
                 c57_base = c57_base + ["T"]
