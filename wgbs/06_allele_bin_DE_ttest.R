...
ref_factor <- opt$ref_factor
if (bin_size %in% c(50, 100, 500)){
  bg_file <- list.files(data.path, pattern = paste0(".bin", bin_size, ".bedgraph"), full.names = F)
} else{
...
for (group in c("total", "c57", "dba")) {
  bedgraphs <- list.files(data.path, pattern = paste0(".*", group, ".bin", bin_size, ".bedgraph"), 
                          full.names = TRUE)
  matrix_list <- list()
  for (bedgraph in bedgraphs) {
    if (file.info(bedgraph)$size > 0) {
      ...
      samplename <- sub(paste0("(.*).", group, ".bin", bin_size, ".bedgraph"), "\\1", basename(bedgraph))
      colnames(bin_value) <- samplename
      matrix_list[[samplename]] <- bin_value
    }
    
  }
  df_list <- lapply(matrix_list, function(mat) {
    as.data.frame(mat) %>%
      rownames_to_column(var = "id")
  })

  matrix <- purrr::reduce(df_list, full_join, by = "id")
  matrix[is.na(matrix)] <- 0
  matrix <- matrix %>% 
    column_to_rownames("id")

  # sampleinfo
  sampleTable <- data.frame(condition = factor(sub("(.*?)-.*", "\\1", colnames(matrix))))
  sampleTable$condition <- relevel(sampleTable$condition, ref = ref_factor)
  ...
  for (comp in comparisons) {
    ...
    comp_sample$condition <- droplevels(comp_sample$condition)
    ...
    pdf(paste0(result.path, "/", group, ".", comp[1], ".vs.", comp[2], 
    ...
    write.csv(diff_res, 
              file = paste0(result.path, "/", group, ".", comp[1], ".vs.", comp[2], 
                            ".bin", bin_size, ".DE.csv"), row.names = F)
  }
}
